...
namespace Sozdanie_Raspisaniya_Proizvodstva_Nomenklatury
{
    ...
    public class tMachineTool
    {
        ...
        public ArrayList arrWorks;          //массив работ (тип tTasks)
        ...
        //возвращает истину, если слесарь может обрабатывать номенклатуру. на вход задается ID номенклатуры.
        ...
        //записать задачу в план обработки слесарем
        public int AssignJob(tTasks Tasks)
        {
            int iTime = GetTimeByNomenclatureID(Tasks._tnNomenclature._id);
            if (iTime != -1)
            {
                iCurrentLoad += iTime;
                arrWorks.Add(Tasks);
    ...
    {
        public tTasks _Tasks;       //ссылка на задачу
        public tMachineTool _Tool;  //ссылка на слесаря
